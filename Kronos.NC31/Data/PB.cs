...
namespace Cronos.SDK.Data
{
    ...
    internal static class PB
    {
        ...
        #region M
        internal static List<byte> I(int t, int l, Bitmap i0, ESLType t0)
        {
            try
            {
                if (i0 == null) { lg.LogError("[SDK]IMG_IS_NUL"); return new List<byte>(); }
                ...
                g.Clear(Color.White);
                g.DrawImage(i0, new Rectangle(0, 0, W, H), new Rectangle(0, 0, W, H), GraphicsUnit.Pixel);
                g.Dispose();
                ...
                unsafe
                {
                    ...
                    for (int h = 0; h < H; ++h)
                    {
                        for (int w = 0; w < W; ++w)
                        {
                            ...
                            {
                                B9[h * W + w] = ((byte)((p[2] * 19595 + p[1] * 38469 + p[0] * 7472) >> 16)) <= 125;
                            }
                            p += 3;
                        }
                        p += offset;
