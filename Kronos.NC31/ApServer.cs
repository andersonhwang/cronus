...
namespace Cronos.SDK
{
    ...
    internal sealed class ApServer
    {
        ...
        // Thread signal
        static Regex REG_EXP = new Regex("[0-9A-F]{2},[01]{1},6,[0-9A-F]{16}V[0-9]{3}V[0-9]{3}.[0-9]{3}[0-9A-F]{8}");
        ...
        readonly internal Dictionary<string, ApClient> Clients = new Dictionary<string, ApClient>();  // Ap clients list   
        ...
        void ConnectCallback(IAsyncResult result)
        {
            lock (_locker)
            {
                ...
                try
                {
                    ...
                    if (ApEvent != null) ApEvent(this, new ApEventArgs(shop, id, client.IP, client.Port, mac,
