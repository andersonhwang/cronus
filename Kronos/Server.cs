...
namespace Cronos.SDK
{
    ...
    public sealed class Server
    {
        ...
        public SdkResult Send(string storeCode, string apID, List<TagEntity> tagEntities)
        {
            try
            {
                if (!Regex.IsMatch(storeCode, "^[0-9A-F]{4}$")) { return SdkResult.InvalidStoreCode; }
                if (!Regex.IsMatch(apID, "^[0-9A-F]{2}$")) { return SdkResult.InvalidApID; }
        ...
        public byte[] GetTagData(
            string tagID, int token, SkiaSharp.SKBitmap bitmap,
